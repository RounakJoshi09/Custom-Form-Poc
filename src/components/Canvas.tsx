...
function DropZone({ columnId, children }: DropZoneProps) {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    accept: [DND_ITEM_TYPES.FIELD_FROM_PALETTE, DND_ITEM_TYPES.EXISTING_FIELD],
    drop: (item: DragItem) => {
...
function Column({ columnId, width }: ColumnProps) {
  ...
  const columnFields = getColumnFields(columnId, state.schema.fields, state.schema.positions);
      ...
      <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
        <Box sx={{ p: 1 }}>
          <Typography variant="caption" color="text.secondary" gutterBottom>
            Column {width}% ({columnConfig.slotsPerRow} per row, {columnConfig.maxRows} rows max)
          </Typography>
        </Box>
        <Divider />
                  ...
                  {Array.from({ length: columnConfig.slotsPerRow }).map((_, slotIndex) => (
                    ...
                    >
                      {row[slotIndex] ? (
                        <FieldCard 
                          field={row[slotIndex]} 
                          isSelected={state.selectedFieldId === row[slotIndex].id}
                        />
                      ) : (
                        <Box 
                          sx={{ 
